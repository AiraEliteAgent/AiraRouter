...
import { PROVIDER_ID_TO_ALIAS } from "@airarouter/open-sse/config/providerModels.ts";
...
export async function getPricingForModel(provider: string, model: string) {
  ...
  const { PROVIDER_ID_TO_ALIAS } = await import("@airarouter/open-sse/config/providerModels");
