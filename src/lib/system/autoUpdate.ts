...
export function getAutoUpdateConfig(env: NodeJS.ProcessEnv = process.env): AutoUpdateConfig {
  const dataDir = env.DATA_DIR || "/tmp/airarouter";
  const repoDir = env.AUTO_UPDATE_REPO_DIR || "/workspace/airarouter";
  ...
  return {
    ...
    composeService: env.AUTO_UPDATE_SERVICE || "airarouter-cli",
...
export async function validateAutoUpdateRuntime(
  ...
  if (!(await existsImpl("/var/run/docker.sock"))) {
    return {
      supported: false,
      reason: "Docker socket is not mounted into the AiraRouter container.",
      composeCommand: null,
    };
  }
  ...
  try {
    ...
    return {
      supported: false,
      reason: "git is not available inside the AiraRouter container.",
      composeCommand: null,
    };
  }
  ...
  if (!composeCommand) {
    return {
      supported: false,
      reason:
        "Neither docker compose nor docker-compose is available inside the AiraRouter container.",
      composeCommand: null,
    };
  }
...
export function buildNpmUpdateScript(latest: string): string {
  return [
    "set -eu",
    `npm install -g airarouter@${latest} --ignore-scripts --legacy-peer-deps`,
    "if command -v pm2 >/dev/null 2>&1; then",
    "  pm2 restart airarouter || true",
    "fi",
