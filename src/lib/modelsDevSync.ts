...
// ─── Provider mapping: models.dev provider ID → AiraRouter provider IDs/aliases ──
//
// models.dev uses canonical provider IDs (e.g. "openai", "anthropic", "google").
// AiraRouter uses both full IDs and short aliases (e.g. "cc" for claude, "cx" for codex).
// We map each models.dev provider to ALL AiraRouter identifiers that should receive
...
const MODELS_DEV_PROVIDER_MAP: Record<string, string[]> = {
  ...
  // Additional providers that may overlap with AiraRouter
...
/**
 * Map a models.dev provider ID to AiraRouter provider IDs.
...
/**
 * Transform models.dev raw data → AiraRouter PricingByProvider format.
 *
 * models.dev costs are already in $/1M tokens (same as AiraRouter format).
...
export function transformModelsDevToPricing(raw: ModelsDevData): PricingByProvider {
  ...
  for (const [providerId, providerData] of Object.entries(raw)) {
    ...
    for (const [modelId, model] of Object.entries(providerData.models || {})) {
      ...
      // Write to ALL mapped AiraRouter providers
