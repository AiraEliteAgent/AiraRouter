...
import { parseModel } from "@airarouter/open-sse/services/model.ts";
import {
  detectFormatFromEndpoint,
  getTargetFormat,
} from "@airarouter/open-sse/services/provider.ts";
import { handleChatCore } from "@airarouter/open-sse/handlers/chatCore.ts";
import { errorResponse, unavailableResponse } from "@airarouter/open-sse/utils/error.ts";
import { handleComboChat } from "@airarouter/open-sse/services/combo.ts";
import { HTTP_STATUS } from "@airarouter/open-sse/config/constants.ts";
import {
  getModelTargetFormat,
  PROVIDER_ID_TO_ALIAS,
} from "@airarouter/open-sse/config/providerModels.ts";
import {
  runWithProxyContext,
  runWithTlsTracking,
  isTlsFingerprintActive,
} from "@airarouter/open-sse/utils/proxyFetch.ts";
...
} from "@airarouter/open-sse/services/taskAwareRouter.ts";
import {
...
} from "@airarouter/open-sse/services/sessionManager.ts";
import {
  isFallbackDecision,
  shouldUseFallback,
} from "@airarouter/open-sse/services/emergencyFallback.ts";
...
function withSessionHeader(response: Response, sessionId: string | null): Response {
  ...
  try {
    response.headers.set("X-AiraRouter-Session-Id", sessionId);
    return response;
  } catch {
    ...
    cloned.headers.set("X-AiraRouter-Session-Id", sessionId);
    return cloned;
  }
}
