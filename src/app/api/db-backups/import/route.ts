...
// Required tables that must exist in a valid AiraRouter database
...
export async function POST(request: Request) {
  ...
  try {
    ...
    tmpPath = path.join(os.tmpdir(), `airarouter-import-${Date.now()}.sqlite`);
    ...
    try {
      ...
      if (missingTables.length > 0) {
        return NextResponse.json(
          {
            error: `Invalid AiraRouter database. Missing tables: ${missingTables.join(", ")}`,
          },
          { status: 400 }
        );
      }
