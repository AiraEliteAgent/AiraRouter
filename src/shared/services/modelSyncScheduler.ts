...
const globalState = globalThis as typeof globalThis & {
  __airarouterModelSyncInternalAuthToken?: string;
};
...
function getInternalAuthToken(): string {
  if (!internalAuthToken) {
    internalAuthToken = globalState.__airarouterModelSyncInternalAuthToken || randomUUID();
    globalState.__airarouterModelSyncInternalAuthToken = internalAuthToken;
  }
...
export function isModelSyncInternalRequest(request: Request): boolean {
  if (!internalAuthToken && globalState.__airarouterModelSyncInternalAuthToken) {
    internalAuthToken = globalState.__airarouterModelSyncInternalAuthToken;
  }
 ...
 * @param apiBaseUrl — internal base URL to call AiraRouter's own API
