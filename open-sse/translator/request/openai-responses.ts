...
export function openaiResponsesToOpenAIRequest(
    ...
    for (const toolValue of tools) {
      ...
      if (toolType && toolType !== "function" && !tool.function) {
        throw unsupportedFeature(
          `Unsupported Responses API feature: ${toolType} tool type is not supported by airarouter`
        );
      }
    }
  }
  ...
  if (root.background) {
    throw unsupportedFeature(
      "Unsupported Responses API feature: background mode is not supported by airarouter"
    );
  }
  ...
  if (
    ...
    if (tcType === "function" && tc.name !== undefined && !tc.function) {
      ...
      throw unsupportedFeature(
        `Unsupported Responses API feature: tool_choice type '${tcType}' is not supported by airarouter`
      );
    }
  }
