/**
 * AiraRouter MCP Server — Model Context Protocol server exposing
 * AiraRouter gateway intelligence as tools for AI agents.
 *
 * Supports two transports:
 *   1. stdio  — for IDE integration (VS Code, Cursor, Claude Desktop)
 *   2. HTTP   — for remote/programmatic access
 *
 * Tools wrap existing AiraRouter API endpoints and add intelligence
...
/**
 * Internal fetch helper that calls AiraRouter API endpoints.
 */
async function omniRouteFetch(path: string, options: RequestInit = {}): Promise<unknown> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`AiraRouter API error [${response.status}]: ${errorText}`);
  }
...
async function handleGetHealth() {
  ...
  try {
    ...
    await logToolCall("airarouter_get_health", {}, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("airarouter_get_health", {}, null, Date.now() - start, false, msg);
...
async function handleListCombos(args: { includeMetrics?: boolean }) {
  ...
  try {
    ...
    await logToolCall("airarouter_list_combos", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("airarouter_list_combos", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleGetComboMetrics(args: { comboId: string }) {
  ...
  try {
    ...
    await logToolCall("airarouter_get_combo_metrics", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("airarouter_get_combo_metrics", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleSwitchCombo(args: { comboId: string; active: boolean }) {
  ...
  try {
    ...
    await logToolCall("airarouter_switch_combo", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("airarouter_switch_combo", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleCheckQuota(args: { provider?: string; connectionId?: string }) {
  ...
  try {
    ...
    await logToolCall("airarouter_check_quota", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("airarouter_check_quota", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleCostReport(args: { period?: string }) {
  ...
  try {
    ...
    await logToolCall("airarouter_cost_report", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("airarouter_cost_report", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleListModelsCatalog(args: { provider?: string; capability?: string }) {
  ...
  try {
    ...
    if (isProviderSpecific) {
      ...
      // AiraRouter's global /v1/models is always a cached/local catalog
    }
    ...
    await logToolCall(
      "airarouter_list_models_catalog",
      args,
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("airarouter_list_models_catalog", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleWebSearch(args: {
  ...
  try {
    ...
    await logToolCall("airarouter_web_search", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("airarouter_web_search", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
/**
 * Create and configure the AiraRouter MCP Server with all essential tools.
 */
export function createMcpServer(): McpServer {
  ...
  server.registerTool(
    ...
    {
      description:
        "Returns AiraRouter health status including uptime, memory, circuit breakers, rate limits, and cache stats",
      inputSchema: getHealthInput,
    },
    withScopeEnforcement("airarouter_get_health", async (args) => {
  ...
  server.registerTool(
    "airarouter_route_request",
    {
      description: "Sends a chat completion request through AiraRouter intelligent routing",
      inputSchema: routeRequestInput,
    },
    withScopeEnforcement("airarouter_route_request", (args) =>
      handleRouteRequest(routeRequestInput.parse(args))
    )
  );

  server.registerTool(
    "airarouter_cost_report",
    {
      description: "Generates a cost report for the specified period",
      inputSchema: costReportInput,
    },
    withScopeEnforcement("airarouter_cost_report", (args) =>
  ...
  server.registerTool(
    ...
    withScopeEnforcement("airarouter_list_models_catalog", (args) =>
  ...
  server.registerTool(
    ...
    withScopeEnforcement("airarouter_set_resilience_profile", (args) =>
      handleSetResilienceProfile(setResilienceProfileInput.parse(args))
    )
  );

  server.registerTool(
    "airarouter_test_combo",
    {
      description:
    ...
    withScopeEnforcement("airarouter_test_combo", (args) =>
      handleTestCombo(testComboInput.parse(args))
    )
  );

  server.registerTool(
    "airarouter_get_provider_metrics",
    {
      description:
  ...
  server.registerTool(
    ...
    withScopeEnforcement("airarouter_explain_route", (args) =>
      handleExplainRoute(explainRouteInput.parse(args))
    )
  );

  server.registerTool(
    "airarouter_get_session_snapshot",
    {
      description:
    ...
    withScopeEnforcement("airarouter_get_session_snapshot", async (args) => {
  ...
  server.registerTool(
    "airarouter_sync_pricing",
    {
      description:
        "Syncs pricing data from external sources (LiteLLM) into AiraRouter without overwriting user-set prices",
      inputSchema: syncPricingInput,
    },
    withScopeEnforcement("airarouter_sync_pricing", (args) =>
      handleSyncPricing(syncPricingInput.parse(args))
    )
  );

  server.registerTool(
    "airarouter_web_search",
    {
      description:
        "Performs a web search using AiraRouter's search gateway. Supports multiple providers (Serper, Brave, Perplexity, Exa, Tavily) with automatic failover. Returns search results with titles, URLs, snippets, and position data.",
      inputSchema: webSearchInput,
    },
    withScopeEnforcement("airarouter_web_search", (args) =>
 ...
 * Called when `airarouter --mcp` is used.
 */
export async function startMcpStdio(): Promise<void> {
  ...
  console.error("[MCP] AiraRouter MCP Server starting (stdio transport)...");
  try {
    await server.connect(transport);
    console.error("[MCP] AiraRouter MCP Server connected and ready.");
  } finally {
    stopHeartbeatOnce();
