# AiraRouter MCP Server

> **Model Context Protocol server** that exposes AiraRouter's gateway intelligence as **16 tools** for AI agents.

The MCP Server allows any AI agent (Claude Desktop, Cursor, VS Code Copilot, custom agents) to **monitor, control, and optimize** the AiraRouter AI gateway programmatically.
...
| --- | -------------------------------- | --------------------- | -------------------------------------------------------------------------- |
| 1   | `airarouter_get_health`          | `read:health`         | Gateway health, uptime, memory, circuit breakers, rate limits, cache stats |
| 2   | `airarouter_list_combos`         | `read:combos`         | List all combos (model chains) with strategies and optional metrics        |
| 3   | `airarouter_get_combo_metrics`   | `read:combos`         | Performance metrics for a specific combo                                   |
| 4   | `airarouter_switch_combo`        | `write:combos`        | Activate or deactivate a combo for routing                                 |
| 5   | `airarouter_check_quota`         | `read:quota`          | Remaining API quota per provider with token health status                  |
| 6   | `airarouter_route_request`       | `execute:completions` | Send a chat completion through intelligent routing                         |
| 7   | `airarouter_cost_report`         | `read:usage`          | Cost report by period (session/day/week/month) with per-provider breakdown |
| 8   | `airarouter_list_models_catalog` | `read:models`         | List all available models across providers with capabilities and pricing   |

### Phase 2: Advanced Tools (8)

| #   | Tool                                | Scopes                               | Description                                                                                    |
| --- | ----------------------------------- | ------------------------------------ | ---------------------------------------------------------------------------------------------- |
| 9   | `airarouter_simulate_route`         | `read:health`, `read:combos`         | Dry-run routing simulation showing fallback tree and estimated costs                           |
| 10  | `airarouter_set_budget_guard`       | `write:budget`                       | Set session budget with action on exceed: `degrade`, `block`, or `alert`                       |
| 11  | `airarouter_set_resilience_profile` | `write:resilience`                   | Apply resilience profile: `aggressive`, `balanced`, or `conservative`                          |
| 12  | `airarouter_test_combo`             | `execute:completions`, `read:combos` | Test each provider in a combo with a real prompt and a real upstream call, report latency/cost |
| 13  | `airarouter_get_provider_metrics`   | `read:health`                        | Per-provider metrics with latency percentiles (p50/p95/p99), circuit breaker                   |
| 14  | `airarouter_best_combo_for_task`    | `read:combos`, `read:health`         | AI-powered combo recommendation by task type with budget/latency constraints                   |
| 15  | `airarouter_explain_route`          | `read:health`, `read:usage`          | Explain why a request was routed to a provider (scoring factors, fallbacks)                    |
| 16  | `airarouter_get_session_snapshot`   | `read:usage`                         | Full session snapshot: cost, tokens, top models, errors, budget status                         |
...
```python
"""
AiraRouter MCP Client — Python example using the mcp SDK.
...
async def main():
    ...
    async with stdio_client(server) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            health = await session.call_tool("airarouter_get_health", {})
            print("Health:", health.content[0].text)

            # 2. List available combos with metrics
            combos = await session.call_tool("airarouter_list_combos", {
                "includeMetrics": True
            })
            print("Combos:", combos.content[0].text)

            # 3. Find the best combo for a coding task
            best = await session.call_tool("airarouter_best_combo_for_task", {
                "taskType": "coding",
            ...
            budget = await session.call_tool("airarouter_set_budget_guard", {
                "maxCost": 1.00,
                "action": "degrade",
                "degradeToTier": "cheap",
            })
            ...
            response = await session.call_tool("airarouter_route_request", {
            ...
            snapshot = await session.call_tool("airarouter_get_session_snapshot", {})
...
async function main() {
  ...
  const simulation = await client.callTool({
    name: "airarouter_simulate_route",
    arguments: {
      model: "claude-sonnet-4",
...
An agent that monitors AiraRouter health and auto-switches combos when providers degrade.
...
async def auto_healing_loop(session):
    ...
    while True:
        ...
        if broken:
            ...
            await session.call_tool("airarouter_set_resilience_profile", {
            ...
            # Activate it
            await session.call_tool("airarouter_switch_combo", {
...
async def budget_aware_coding(session, task: str, max_budget: float):
    ...
    # Set budget guard
    await session.call_tool("airarouter_set_budget_guard", {
        "maxCost": max_budget,
        "action": "degrade",
        "degradeToTier": "cheap",
    })
    ...
    sim = await session.call_tool("airarouter_simulate_route", {
        "model": "claude-sonnet-4",
    ...
    # Send request
    result = await session.call_tool("airarouter_route_request", {
        "model": "claude-sonnet-4",
...
async def benchmark_combos(session):
    ...
    for combo in combo_list:
        ...
        test = await session.call_tool("airarouter_test_combo", {
            "comboId": combo["id"],
...
async def find_cheapest_models(session, capability="chat"):
    ...
    catalog = await session.call_tool("airarouter_list_models_catalog", {
