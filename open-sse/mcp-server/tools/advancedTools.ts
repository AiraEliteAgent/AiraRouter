/**
 * AiraRouter MCP Advanced Tools — 10 intelligence tools that differentiate
 * AiraRouter from all other AI gateways.
 *
 * Tools:
 *   1. airarouter_simulate_route     — Dry-run routing simulation
 *   2. airarouter_set_budget_guard   — Session budget with degrade/block/alert
 *   3. airarouter_set_routing_strategy — Runtime strategy switch for combos
 *   4. airarouter_set_resilience_profile — Circuit breaker/retry profiles
 *   5. airarouter_test_combo         — Live test each provider in a combo
 *   6. airarouter_get_provider_metrics — Detailed per-provider metrics
 *   7. airarouter_best_combo_for_task — AI-powered combo recommendation
 *   8. airarouter_explain_route      — Post-hoc routing decision explainer
 *   9. airarouter_get_session_snapshot — Full session state snapshot
 *  10. airarouter_sync_pricing      — Sync provider pricing from external source
 */
...
export async function handleSimulateRoute(args: {
  ...
  try {
    ...
    await logToolCall("airarouter_simulate_route", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("airarouter_simulate_route", args, null, Date.now() - start, false, msg);
...
export async function handleSetBudgetGuard(args: {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("airarouter_set_budget_guard", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleSetRoutingStrategy(args: {
  ...
  try {
    ...
    await logToolCall("airarouter_set_routing_strategy", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall(
...
export async function handleSetResilienceProfile(args: {
  ...
  try {
    ...
    // Apply to AiraRouter via API (contract: PATCH + { profiles, defaults })
    ...
    await logToolCall("airarouter_set_resilience_profile", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall(
...
export async function handleTestCombo(args: { comboId: string; testPrompt: string }) {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("airarouter_test_combo", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleGetProviderMetrics(args: { provider: string }) {
  ...
  try {
    ...
    await logToolCall("airarouter_get_provider_metrics", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall(
...
export async function handleBestComboForTask(args: {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("airarouter_best_combo_for_task", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleExplainRoute(args: { requestId: string }) {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("airarouter_explain_route", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleSyncPricing(args: { sources?: string[]; dryRun?: boolean }) {
  ...
  try {
    ...
    await logToolCall("airarouter_sync_pricing", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("airarouter_sync_pricing", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleGetSessionSnapshot() {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("airarouter_get_session_snapshot", {}, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
