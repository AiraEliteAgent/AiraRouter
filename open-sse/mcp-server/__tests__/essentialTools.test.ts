 ...
 * The airarouter_web_search tests use InMemoryTransport + Client to exercise
...
describe("MCP Essential Tools", () => {
  ...
  describe("Tool schema validation", () => {
    ...
    it("all tools should have airarouter_ prefix", () => {
      const schemas = MCP_ESSENTIAL_TOOLS;
      for (const schema of schemas) {
        expect(schema.name).toMatch(/^airarouter_/);
      }
    });
  });
...
// ── airarouter_web_search: handler dispatch tests ──────────────────────────────
...
describe("airarouter_web_search handler (via MCP dispatch)", () => {
  let client: Client;
  ...
  it("should appear in tools/list after registration", async () => {
    ...
    const webSearch = tools.find((t) => t.name === "airarouter_web_search");
