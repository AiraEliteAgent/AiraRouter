...
export class GeminiCLIExecutor extends BaseExecutor {
   ...
   * Native Gemini CLI refreshes this every 30 seconds — AiraRouter stores it once
  ...
  async _doRefresh(accessToken: string): Promise<string | null> {
    try {
      ...
      if (!response.ok) {
        console.warn(
          `[AiraRouter] loadCodeAssist returned ${response.status} — falling back to stored projectId`
        );
        return null;
      }
      ...
      if (!projectId) {
        console.warn(
          "[AiraRouter] loadCodeAssist returned no project — falling back to stored projectId"
        );
        return null;
      }
      ...
      console.warn(
        `[AiraRouter] loadCodeAssist failed (${msg}) — falling back to stored projectId`
      );
      return null;
    }
  }
