/**
 * AiraRouter Electron Desktop App - Main Process
 *
 ...
 * #14 Removed dead airarouter:// protocol (no handler existed)
...
function resolveDataDir(overridePath, env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return path.join(appData, "airarouter");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return path.join(path.resolve(xdg), "airarouter");

  return path.join(require("os").homedir(), ".airarouter");
}
...
function setupContentSecurityPolicy() {
  session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
    const csp = [
      "default-src 'self'",
      `connect-src 'self' http://localhost:* ws://localhost:* https://*.airarouter.online https://*.airarouter.dev`,
...
function startNextServer() {
  ...
  if (changed) {
    ...
    try {
      ...
      const lines = [
        "# Auto-generated by AiraRouter bootstrap",
        "",
