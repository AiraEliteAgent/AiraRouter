  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        expect(toolNames).toContain("airarouter_get_health");
        expect(toolNames).toContain("airarouter_list_combos");
      ...
      const auditRes = await apiFetch("/api/mcp/audit?limit=50&tool=airarouter_get_health");
      if (auditRes.status === 401) {
        ...
        expect(entries.some((entry: any) => entry.toolName === "airarouter_get_health")).toBe(true);
      }
    },
    TEST_TIMEOUT_MS * 2
  );
