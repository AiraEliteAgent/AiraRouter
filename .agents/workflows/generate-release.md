...
gh release create "v$VERSION" --title "v$VERSION" --notes "AiraRouter v$VERSION Release" --target main
```
...
gh run list --repo diegosouzapw/AiraRouter --workflow docker-publish.yml --limit 3

# Wait for the Docker build to complete (usually 5–10 min)
gh run watch --repo diegosouzapw/AiraRouter

# After completion, verify on Docker Hub:
# https://hub.docker.com/r/diegosouzapw/airarouter/tags
```

If the Docker build was not triggered automatically, trigger it manually:

```bash
gh workflow run docker-publish.yml --repo diegosouzapw/AiraRouter --ref v2.x.y
```
...
scp airarouter-*.tgz root@192.168.0.15:/tmp/
...
scp airarouter-*.tgz root@69.164.221.35:/tmp/
...
- After npm publish, verify with `npm info airarouter version`
