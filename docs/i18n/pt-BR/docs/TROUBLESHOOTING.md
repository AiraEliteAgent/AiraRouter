...
Common problems and solutions for AiraRouter.
...
| EACCES: permission denied     | Set `DATA_DIR=/path/to/writable/dir` to override `~/.airarouter`   |
...
AiraRouter auto-refreshes tokens. If issues persist:
...
2. Verify `CLOUD_URL` points to your cloud endpoint (e.g., `https://airarouter.dev`)
...
When many concurrent requests hit a rate-limited provider, AiraRouter uses mutex + auto rate-limiting to serialize requests and prevent cascading failures. This is automatic for API key providers.

---

## Optional RAG / LLM failure taxonomy (16 problems)

Some AiraRouter users place the gateway in front of RAG or agent stacks. In those setups it is common to see a strange pattern: AiraRouter looks healthy (providers up, routing profiles ok, no rate limit alerts) but the final answer is still wrong.
...
1. When you investigate a bad response, capture:
   ...
   - route or provider combo in AiraRouter
   - any RAG context used downstream (retrieved documents, tool calls, etc)
2. Map the incident to one or two WFGY ProblemMap numbers (`No.1` … `No.16`).
3. Store the number in your own dashboard, runbook, or incident tracker next to the AiraRouter logs.
...
You can ignore this section if you do not run RAG or agent pipelines behind AiraRouter.

---

## Still Stuck?

- **GitHub Issues**: [github.com/diegosouzapw/AiraRouter/issues](https://github.com/diegosouzapw/AiraRouter/issues)
