...
Complete guide for configuring providers, creating combos, integrating CLI tools, and deploying AiraRouter.
...
```json
{
  ...
  "anthropic_api_key": "your-airarouter-api-key"
}
```
...
```json
{
  "agents": {
    "defaults": {
      "model": { "primary": "airarouter/if/glm-4.7" }
    }
  },
  "models": {
    "providers": {
      "airarouter": {
        ...
        "apiKey": "your-airarouter-api-key",
...
cp .env.example ~/.airarouter/.env

# Start server
airarouter
# Or with custom port:
airarouter --port 3000
```

The CLI automatically loads `.env` from `~/.airarouter/.env` or `./.env`.

### VPS Deployment

```bash
git clone https://github.com/diegosouzapw/AiraRouter.git
cd AiraRouter && npm install && npm run build

export JWT_SECRET="your-secure-secret-change-this"
export INITIAL_PASSWORD="your-password"
export DATA_DIR="/var/lib/airarouter"
export PORT="20128"
...
npm run start
# Or: pm2 start npm --name airarouter -- start
```
...
pm2 start npm --name airarouter -- start

# Or with custom memory limit
OMNIROUTE_MEMORY_MB=512 pm2 start npm --name airarouter -- start
...
docker build -t airarouter:cli .

# Portable mode (recommended)
docker run -d --name airarouter -p 20128:20128 --env-file ./.env -v airarouter-data:/app/data airarouter:cli
```
...
Void Linux users can package and install AiraRouter natively using the `xbps-src` cross-compilation framework. This automates the Node.js standalone build along with the required `better-sqlite3` native bindings.
...
| --------------------------------------- | ------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `JWT_SECRET`                            | `airarouter-default-secret-change-me` | JWT signing secret (**change in production**)                                                             |
| `INITIAL_PASSWORD`                      | `123456`                              | First login password                                                                                      |
| `DATA_DIR`                              | `~/.airarouter`                       | Data directory (db, usage, logs)                                                                          |
| `PORT`                                  | framework default                     | Service port (`20128` in examples)                                                                        |
| `HOSTNAME`                              | framework default                     | Bind host (Docker defaults to `0.0.0.0`)                                                                  |
| `NODE_ENV`                              | runtime default                       | Set `production` for deploy                                                                               |
| `BASE_URL`                              | `http://localhost:20128`              | Server-side internal base URL                                                                             |
| `CLOUD_URL`                             | `https://airarouter.dev`              | Cloud sync endpoint base URL                                                                              |
...
- **Semantic Cache** — Auto-caches non-streaming, temperature=0 responses (bypass with `X-AiraRouter-No-Cache: true`)
- **Request Idempotency** — Deduplicates requests within 5s via `Idempotency-Key` or `X-Request-Id` header
- **Progress Tracking** — Opt-in SSE `event: progress` events via `X-AiraRouter-Progress: true` header

---

### Translator Playground

Access via **Dashboard → Translator**. Debug and visualize how AiraRouter translates API requests between providers.
...
AiraRouter also accepts `x_session_id` and returns the effective session key in `X-AiraRouter-Session-Id`.
...
AiraRouter implements provider-level resilience with four components:
...
- Migrate AiraRouter between machines
...
AiraRouter supports audio transcription via the OpenAI-compatible endpoint:
...
AiraRouter is available as a native desktop application for Windows, macOS, and Linux.
