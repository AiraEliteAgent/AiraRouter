# CLI Tools Setup Guide — AiraRouter (Português (Portugal))
...
to use **AiraRouter** as the unified backend, giving you centralized key management,
...
Claude / Codex / OpenCode / Cline / KiloCode / Continue / Kiro / Cursor / Copilot
           │
           ▼  (all point to AiraRouter)
    http://YOUR_SERVER:20128/v1
           │
           ▼  (AiraRouter routes to the right provider)
...
| **Antigravity**    | `antigravity` | internal   | mitm       | AiraRouter     |
...
## Step 1 — Get an AiraRouter API Key

1. Open the AiraRouter dashboard → **API Manager** (`/dashboard/api-manager`)
...
```bash
# AiraRouter Universal Endpoint
export OPENAI_BASE_URL="http://localhost:20128/v1"
export OPENAI_API_KEY="sk-your-airarouter-key"
export ANTHROPIC_BASE_URL="http://localhost:20128/v1"
export ANTHROPIC_API_KEY="sk-your-airarouter-key"
export GEMINI_BASE_URL="http://localhost:20128/v1"
export GEMINI_API_KEY="sk-your-airarouter-key"
```
...
api_key = "sk-your-airarouter-key"
EOF
```
...
mkdir -p ~/.cline/data && cat > ~/.cline/data/globalState.json << EOF
{
  ...
  "openAiApiKey": "sk-your-airarouter-key"
}
EOF
```
...
Or use the AiraRouter dashboard → **CLI Tools → Cline → Apply Config**.
...
```bash
kilocode --api-base http://localhost:20128/v1 --api-key sk-your-airarouter-key
```
...
```json
{
  ...
  "kilo-code.apiKey": "sk-your-airarouter-key"
}
```

Or use the AiraRouter dashboard → **CLI Tools → KiloCode → Apply Config**.
...
# The CLI uses its own auth — AiraRouter is not needed as backend for Kiro CLI itself.
# Use kiro-cli alongside AiraRouter for other tools.
kiro-cli status
```
...
> **Note:** Cursor routes requests through its cloud. For AiraRouter integration,
> enable **Cloud Endpoint** in AiraRouter Settings and use your public domain URL.

Via GUI: **Settings → Models → OpenAI API Key**

- Base URL: `https://your-domain.com/v1`
- API Key: your AiraRouter key

---

## Dashboard Auto-Configuration

The AiraRouter dashboard automates configuration for most tools:
...
**Droid** and **OpenClaw** are AI agents built directly into AiraRouter — no installation needed.
They run as internal routes and use AiraRouter's model routing automatically.
...
| Error                     | Cause                   | Fix                                        |
| ------------------------- | ----------------------- | ------------------------------------------ |
| `Connection refused`      | AiraRouter not running  | `pm2 start airarouter`                     |
...
```bash
# Install all CLIs and configure for AiraRouter (replace with your key and server URL)
OMNIROUTE_URL="http://localhost:20128/v1"
OMNIROUTE_KEY="sk-your-airarouter-key"
...
source ~/.bashrc
echo "✅ All CLIs installed and configured for AiraRouter"
```
