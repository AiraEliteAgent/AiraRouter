# AiraRouter Architecture (Português (Portugal))
...
AiraRouter is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[AiraRouter Local Process]
...
- file: `${DATA_DIR}/storage.sqlite` (or `$XDG_CONFIG_HOME/airarouter/storage.sqlite` when set, else `~/.airarouter/storage.sqlite`)
...
flowchart LR
    ...
    subgraph ContainerOrProcess[AiraRouter Runtime]
        Next[Next.js Server\nPORT=20128]
...
- final client response returned by AiraRouter; streamed responses are stored in the same compact summary form
...
1. `usageDb` and `localDb` share the same base directory policy (`DATA_DIR` -> `XDG_CONFIG_HOME/airarouter` -> `~/.airarouter`) with legacy file migration.
...
5. The `open-sse/` directory is published as the `@airarouter/open-sse` **npm workspace package**. Source code imports it via `@airarouter/open-sse/...` (resolved by Next.js `transpilePackages`). File paths in this document still use the directory name `open-sse/` for consistency.
...
- Build Docker image: `docker build -t airarouter .`
