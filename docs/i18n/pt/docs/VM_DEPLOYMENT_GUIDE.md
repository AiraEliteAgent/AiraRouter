# AiraRouter — Deployment Guide on VM with Cloudflare (Português (Portugal))
...
Complete guide to install and configure AiraRouter on a VM (VPS) with domain managed via Cloudflare.
...
## 2. Install AiraRouter

### 2.1 Create configuration directory

```bash
mkdir -p /opt/airarouter
```

### 2.2 Create environment variables file

```bash
cat > /opt/airarouter/.env << ‘EOF’
# === Security ===
...
# === Cloud Sync (optional) ===
# CLOUD_URL=https://cloud.airarouter.online
# NEXT_PUBLIC_CLOUD_URL=https://cloud.airarouter.online
EOF
```
...
```bash
docker pull diegosouzapw/airarouter:latest

docker run -d \
  --name airarouter \
  --restart unless-stopped \
  --env-file /opt/airarouter/.env \
  -p 20128:20128 \
...
```bash
docker ps | grep airarouter
docker logs airarouter --tail 20
```
...
```bash
cat > /etc/nginx/sites-available/airarouter << ‘NGINX’
...
Keep reverse-proxy stream timeouts aligned with your AiraRouter timeout env vars. If you raise
...
# Enable AiraRouter
ln -sf /etc/nginx/sites-available/airarouter /etc/nginx/sites-enabled/airarouter
...
```bash
docker pull diegosouzapw/airarouter:latest
docker stop airarouter && docker rm airarouter
docker run -d --name airarouter --restart unless-stopped \
  --env-file /opt/airarouter/.env \
  -p 20128:20128 \
  -v airarouter-data:/app/data \
  diegosouzapw/airarouter:latest
...
```bash
docker logs -f airarouter          # Real-time stream
docker logs airarouter --tail 50   # Last 50 lines
```
...
docker cp airarouter:/app/data ./backup-$(date +%F)

# Or compress the entire volume
docker run --rm -v airarouter-data:/data -v $(pwd):/backup \
  alpine tar czf /backup/airarouter-data-$(date +%F).tar.gz /data
```

### Restore from backup

```bash
docker stop airarouter
docker run --rm -v airarouter-data:/data -v $(pwd):/backup \
  alpine sh -c “rm -rf /data/* && tar xzf /backup/airarouter-data-YYYY-MM-DD.tar.gz -C /”
docker start airarouter
```
...
See the full documentation at [airarouterCloud/README.md](../airarouterCloud/README.md).
...
| 20128 | AiraRouter  | Localhost only (via nginx) |
