# AiraRouter Fly.io 部署指南 (Português (Portugal))
...
本文档记录 AiraRouter 在 Fly.io 上的实际部署方法，适用于两类场景：

- 首次把当前项目部署到 Fly.io
- 后续代码更新后继续发布
- 新项目参考同样流程部署

本文基于当前项目已经验证通过的配置整理，应用名为 `airarouter`。
...
- `app = 'airarouter'` 决定实际部署到哪个 Fly 应用
...
```powershell
git clone https://github.com/xiaoge1688/AiraRouter.git
cd AiraRouter
```
...
```toml
app = 'airarouter-yourname'
```

注意：

- 控制台里要看的是与 `fly.toml` 里 `app` 一致的应用
- 以前如果用过别的名字，例如 `oroute`，不要和 `airarouter` 混淆
...
```powershell
flyctl apps create airarouter
```

如果你已经改成别的应用名，把 `airarouter` 替换成你的名字。
...
| ---------------------- | ---------------------------- |
| `DATA_DIR`             | `/data`                      |
| `NEXT_PUBLIC_BASE_URL` | `https://airarouter.fly.dev` |
...
```powershell
flyctl secrets set INITIAL_PASSWORD=你的强密码 -a airarouter
```
...
```powershell
flyctl secrets list -a airarouter
```
...
```powershell
flyctl secrets set KEY=value -a airarouter
```
...
如果当前仓库是 fork，并且你要同步上游 `https://github.com/diegosouzapw/AiraRouter` 的更新，推荐按下面流程执行。
...
```powershell
git remote add upstream https://github.com/diegosouzapw/AiraRouter.git
```
...
5. `flyctl deploy`
6. `flyctl status -a airarouter`
7. `flyctl logs --no-tail -a airarouter`
...
```powershell
flyctl status -a airarouter
```

### 10.2 查看启动日志

```powershell
flyctl logs --no-tail -a airarouter
```
...
```powershell
try {
  (Invoke-WebRequest -Uri "https://airarouter.fly.dev" -MaximumRedirection 5 -UseBasicParsing).StatusCode
} catch {
...
- 你打开的是另一个应用，例如 `oroute`，不是 `airarouter`
...
1. `flyctl auth login`
2. `flyctl apps create airarouter`
3. `flyctl secrets set ... -a airarouter`
4. `flyctl deploy`
5. `flyctl logs --no-tail -a airarouter`
