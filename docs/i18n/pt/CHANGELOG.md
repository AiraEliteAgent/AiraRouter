...
- **Ecosystem Proxies:** Completed explicit integration mapping model synchronization schedulers, OAuth cycles, and Token Check refreshes safely through AiraRouter's native system upstream proxies (#953).
- **MCP Extensibility:** Added and successfully registered the new `airarouter_web_search` MCP framework tool out of beta into production schemas (#951).
...
- **Cloudflare Quick Tunnels State:** Fixed a severe state inconsistency where restarted AiraRouter instances erroneously showed destroyed tunnels as active, and defaulted cloudflared tunneling to `HTTP/2` to eliminate UDP receive buffer log spam (#925).
...
- **Web Search MCP:** Added the `airarouter_web_search` tool with the `execute:search` scope.
...
> On the first startup after upgrading, AiraRouter archives legacy request logs from `DATA_DIR/logs/`, legacy `DATA_DIR/call_logs/`, and `DATA_DIR/log.txt` into `DATA_DIR/log_archives/*.zip`, then removes the deprecated layout and switches to the new unified artifact format under `DATA_DIR/call_logs/`.
...
- **Admin Updater:** Dynamic source-installation detection for the dashboard Updater. Safely disables the `Update Now` button when AiraRouter is built locally rather than through npm, prompting for `git pull` (#743).
...
- **Void Linux Deployment Support (#732)** — Integrated `xbps-src` packaging template and instructions to natively compile and install AiraRouter with `better-sqlite3` bindings via cross-compilation target.
...
- **Global Fallback Provider (#689)** — When all combo models are exhausted (502/503), AiraRouter now attempts a configurable global fallback model before returning the error. Set `globalFallbackModel` in settings to enable.
...
- **Fix #708** — Prevented token bleeding for Claude Code users routing through AiraRouter by correctly preserving native \`cache_control\` headers during Claude-to-Claude passthrough (PR #708 by @tombii)
...
- **Claude Prompt Caching Passthrough** — Fixed cache_control markers being stripped in Claude passthrough mode (Claude → AiraRouter → Claude), which caused Claude Code users to deplete their Anthropic API quota 5-10x faster than direct connections. AiraRouter now preserves client's cache_control markers when sourceFormat and targetFormat are both Claude, ensuring prompt caching works correctly and dramatically reducing token consumption.
...
- **Kiro Account Ban Documentation** — Documented as upstream AWS anti-fraud false positive, not an AiraRouter issue (#649)
...
### 🎉 AiraRouter v3.0.0 — The Free AI Gateway, Now with 67+ Providers
...
Auto-generate and issue AiraRouter API keys programmatically with per-provider and per-account quota enforcement.
...
```bash
# npm
npm install -g airarouter@3.0.0

# Docker
docker pull diegosouzapw/airarouter:3.0.0
...
AiraRouter now automatically refreshes model lists for connected providers every **24 hours**.
...
- **#537 — Gemini CLI OAuth:** Clear actionable error when `GEMINI_OAUTH_CLIENT_SECRET` is missing in Docker/self-hosted deployments. Previously showed cryptic `client_secret is missing` from Google. Now provides specific `docker-compose.yml` and `~/.airarouter/.env` instructions.
...
- **#492 — CLI startup:** `airarouter` CLI now detects `mise`/`nvm`-managed Node when `app/server.js` is missing and shows targeted fix instructions.
...
- **T04** — `X-Session-Id` header for external sticky routing: `extractExternalSessionId()` in `sessionManager.ts` reads `x-session-id` / `x-airarouter-session` headers with `ext:` prefix to avoid collision with internal SHA-256 session IDs. Nginx-compatible (hyphenated header).
...
- **#492** — `airarouter` CLI now detects `mise`/`nvm` when `app/server.js` is missing and shows targeted fix
...
- **#485** — Kiro MCP tool calls: closed as external Kiro issue (not AiraRouter)
...
- **feat(mcp)**: 2 new advanced tool schemas: `airarouter_get_provider_metrics` (p50/p95/p99 per provider) and `airarouter_explain_route` (routing decision explanation)
- **feat(mcp)**: MCP tool auth scopes updated — `metrics:read` scope added for provider metrics tools
- **feat(mcp)**: `airarouter_best_combo_for_task` now accepts `languageHint` parameter for multilingual routing
...
- **fix(startup) #369**: Unify env loading across `npm run start`, `run-standalone.mjs`, and Electron to respect `DATA_DIR/.env → ~/.airarouter/.env → ./.env` priority — prevents generating a new `STORAGE_ENCRYPTION_KEY` over an existing encrypted database
...
- **External Pricing Sync (LiteLLM)**: 3-tier pricing resolution (user overrides → synced → defaults). Opt-in via `PRICING_SYNC_ENABLED=true`. MCP tool `airarouter_sync_pricing`. 23 new tests.
...
- **GeminiCLI missing projectId (#338)**: Instead of throwing a hard 500 error when `projectId` is missing from stored credentials (e.g. after Docker restart), AiraRouter now logs a warning and attempts the request — returning a meaningful provider-side error instead of an AiraRouter crash.
...
- **Intra-Family Model Fallback (T5)**: When a model is unavailable (404/400/403), AiraRouter now automatically falls back to sibling models from the same family before returning an error (`modelFamilyFallback.ts`).
