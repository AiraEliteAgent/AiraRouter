...
AiraRouter supports **30 languages** with full dashboard UI translation, translated documentation, and RTL support for Arabic and Hebrew.
...
**Secondary translator** — uses any OpenAI-compatible LLM API (including AiraRouter itself) to translate existing `docs/i18n/` markdown files. Best for polishing or re-translating docs with better quality than Google Translate.
